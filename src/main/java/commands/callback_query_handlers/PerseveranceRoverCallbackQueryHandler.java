 ...
 * {@link buttons.InlineButtonsWithMarsRoverOption#PERSEVERANCE} callback query.
 */
public class PerseveranceRoverCallbackQueryHandler implements CallbackQueryHandlerInterface {
    /**
     * Handles {@link buttons.InlineButtonsWithMarsRoverOption#PERSEVERANCE} callback query received by
     ...
     * {@link MarsRover#MARS_PERSEVERANCE} rover information respectively.
     *
    ...
    public void handleCallbackQuery(String callbackData, Long chatId, AstroBot bot) {
        if (bot.isRoverCalled()) {
            ...
            bot.setPerseveranceCalled(true);
            try {
            ...
            bot.deleteMessage(chatId, bot.getLastMessageId());
            bot.sendMessage(chatId, bot.getMarsRover().getRoverInfo(MarsRover.MARS_PERSEVERANCE));
